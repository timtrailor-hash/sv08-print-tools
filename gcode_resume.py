...
def get_checkpoint_layer():
    """Read last checkpoint and return suggested resume layer.

    Uses time-based extrapolation when the checkpoint is stale (daemon
    was down during crash). Falls back to raw checkpoint layer when
    extrapolation data isn't available.

    Logic:
    1. If checkpoint timestamp < 60s old → use current_layer directly
    2. If stale → extrapolate using avg_layer_duration_s + elapsed time
    3. Apply -2 safety margin for adhesion
    """
    try:
    ...
    cp_time_str = cp.get("timestamp", "")
    avg_duration = cp.get("avg_layer_duration_s", 0)
    last_change_str = cp.get("last_layer_change_time", "")

    print(f"Checkpoint: layer {last_layer}, Z={cp.get('z_position')}mm, "
          f"{cp.get('progress_pct', 0):.1f}%")

    # Determine checkpoint age
    estimated_layer = last_layer
    try:
        from datetime import datetime as dt
        cp_time = dt.fromisoformat(cp_time_str)
        age_s = (dt.now() - cp_time).total_seconds()
        print(f"  Checkpoint age: {age_s:.0f}s")

        if age_s < 60:
            # Fresh checkpoint — daemon was running recently
            print(f"  Fresh checkpoint (< 60s) — using layer {last_layer} directly")
        elif avg_duration > 0 and last_change_str:
            # Stale checkpoint — extrapolate from last layer change
            last_change = dt.fromisoformat(last_change_str)
            elapsed = (dt.now() - last_change).total_seconds()
            extra_layers = elapsed / avg_duration
            estimated_layer = last_layer + extra_layers
            print(f"  Stale checkpoint — extrapolating:")
            print(f"    Last layer change: {last_change_str}")
            print(f"    Elapsed since: {elapsed:.0f}s")
            print(f"    Avg layer duration: {avg_duration:.1f}s")
            print(f"    Estimated extra layers: {extra_layers:.1f}")
            print(f"    Estimated current layer: {estimated_layer:.0f}")
        else:
            print(f"  Stale checkpoint but no timing data — using raw layer {last_layer}")
    except (ValueError, TypeError):
        print(f"  Cannot parse checkpoint timestamp — using raw layer {last_layer}")

    safe_layer = max(1, int(estimated_layer) - 2)
    print(f"Resuming from layer {safe_layer} (estimated {int(estimated_layer)} minus 2 for adhesion)")
    return safe_layer
...
def parse_and_split(resp, total_bytes, target_layer):
    ...
    last_e = 0.0
    target_z = 0.0
    relative_extrude = False  # M83=True, M82=False
    ...
    for raw_line in resp:
            ...
            if stripped.startswith("LAYER_CHANGE"):
                ...
                if cur_layer == target_layer:
                    ...
                    target_state = {
                        ...
                        "relative_extrude": relative_extrude,
                    }
                    found_target = True
        ...
        if line.startswith("M83"):
            relative_extrude = True
        elif line.startswith("M82"):
            relative_extrude = False
...
def generate_resume_gcode(state, remaining_buf, total_layers=0, object_defines=None):
    ...
    layer = state["layer"]
    relative_extrude = state.get("relative_extrude", False)

    lines = []
    lines.append(f"; RESUME FROM LAYER {layer} (Z={z:.2f}mm)")
    lines.append(f"; Generated by gcode_resume.py at {time.strftime('%Y-%m-%d %H:%M:%S')}")
    extrude_mode = "relative (M83)" if relative_extrude else "absolute (M82)"
    lines.append(f"; Original E={e:.3f} F={feedrate} fan={fan} extrude={extrude_mode}")
    lines.append("")
    ...
    # Restore extrusion mode to match slicer
    lines.append("; === STATE RESTORATION ===")
    if relative_extrude:
        lines.append("M83                        ; relative extruder (matches slicer)")
        # No G92 E needed — relative mode doesn't track absolute E position
    else:
